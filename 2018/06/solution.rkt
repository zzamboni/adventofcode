...
(define points
  (map (lambda (s) (map string->number (string-split s #rx", ")))
       (file->lines "test-input.txt")))

(define min-x (sub1 (apply min (map first points))))
(define max-x (add1 (apply max (map first points))))
(define min-y (sub1 (apply min (map second points))))
(define max-y (add1 (apply max (map second points))))
...
(for* ([x (in-range min-x (add1 max-x))]
  ...
  (let ([coord (list x y)])
    ...
    (let-values ([(closest-distance closest-points)
                  (for/fold 
                            ([min-d 1000]
                             [min-p '()])
                            ([p points])
      ...
      (when (= 1 (length closest-points))
             (begin
               ...
               (set-point! point-map x y (list closest-distance coord))
               (when (or (= x min-x) (= x max-x) (= y min-y) (= y max-y)) (hash-set! disqualified (first closest-points) #t)))
            ))))
(printf "~n")
(printf "~a~n" closest-map)
(printf "~a~n" disqualified)